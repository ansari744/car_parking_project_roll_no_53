...
public class RCC_UIDashboardButton : MonoBehaviour {
	...
	//public Sprite gearForward, Reverse;
	//Image img;
	public int gearDirection = 0;

	public Button GastBtn;
	public Sprite ForwardSprite, forwardPressed,ReverseSprite,ReversePressed;
	public GameObject DriveBtnTutorial;
	void Start(){
	...
	bool dir = true;
    public GameObject[] parkingBtns;
    public Image[] parkingText;
	public void ChangeGear(int no)
	{
        gearDirection = no;
        for (int i = 0; i < parkingBtns.Length; i++)
        {
            parkingBtns[i].SetActive(false);
        }
      //  parkingBtns[no].SetActive(true);
        for (int i = 0; i < parkingText.Length; i++)
        {
            parkingText[i].color = Color.white;
            parkingText[i].transform.localScale = new Vector3(1f,1f,1f);
        }
        //parkingText[no].color = Color.green;
        //parkingText[no].transform.localScale = new Vector3(1.5f, 1.5f, 1.5f);
        if (no==0) 
		{
			parkingBtns[no].SetActive(true);
			parkingText[no].color = Color.green;
			parkingText[no].transform.localScale = new Vector3(1.5f, 1.5f, 1.5f);
			GastBtn.GetComponent<Image>().sprite = ForwardSprite;
			SpriteState ss = new SpriteState();
			ss.pressedSprite = forwardPressed;
			GastBtn.spriteState = ss;
		} 
		else if(no==2)
        {
			parkingBtns[1].SetActive(true);
			parkingText[1].color = Color.green;
			parkingText[1].transform.localScale = new Vector3(1.5f, 1.5f, 1.5f);
			GastBtn.GetComponent<Image>().sprite = ReverseSprite;
			SpriteState ss = new SpriteState();
			ss.pressedSprite = ReversePressed;
			GastBtn.spriteState = ss;
		}
		if (!RCC_SceneManager.Instance.activePlayerVehicle)
			return;
		...
		switch(gearDirection)
		{
			case 0:
				...
				RCC_SceneManager.Instance.activePlayerVehicle.NGear = false;
					DriveBtnTutorial.SetActive(false);
					break;
