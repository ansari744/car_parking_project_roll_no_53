...
public class RCC_CarControllerV3 : MonoBehaviour
{
	...
	public RCC_UIDashboardButton DashBoardButton;
...
//		GetComponentInChildren<WheelCollider>().ConfigureVehicleSubsteps(10f, 1, 1);
		...
		switch(RCCSettings.behaviorType)
		{
			...
			case RCC_Settings.BehaviorType.Drift:
				...
				rigid.angularDamping = Mathf.Clamp(rigid.angularDamping, .5f, 1f);
				break;
...
//					engineSoundHigh.pitch = 0f;

					engineSoundHighOff.volume =0f;
//					engineSoundHighOff.pitch =0f;
				} 
				else 
				{
	...
	void SteerHelper()
	{
		...
		rigid.AddRelativeTorque (Vector3.up * ((angle2 * (Mathf.Clamp(transform.InverseTransformDirection(rigid.linearVelocity).z, -10f, 10f) / 600f)) * steerHelperAngularVelStrength), ForceMode.VelocityChange);
	...
	void GearBox ()
	{
		...
		if(direction == -1)
		{
			autoGenerateGearCurves = false;
			autoGenerateTargetSpeedsForChangingGear = false;
			currentGear = 3;
		...
		{
			autoGenerateGearCurves = true;
			autoGenerateTargetSpeedsForChangingGear = true;
