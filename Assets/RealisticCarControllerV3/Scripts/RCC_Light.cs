		...
		switch(lightType)
		{
			...
			case LightType.BrakeLight:
				if (parkLightFound)
					...
					Lighting(carController._brakeInput >= .1f ? 0.4f : !carController.lowBeamHeadLightsOn ? 0f : .25f);
				break;
	...
	void Indicators(){
		...
		switch(indicatorsOn)
		{
			case RCC_CarControllerV3.IndicatorsOn.Left:
				...
				{
					Lighting (1);
					if(!indicatorSound.isPlaying && carController.indicatorTimer <= .05f && PlayerPrefs.GetInt("sound") == 0)
						indicatorSound.Play();
				}
